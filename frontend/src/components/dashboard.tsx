...
export function Dashboard() {
  ...
  return (
    <main className="mx-auto max-w-[1440px] px-5 py-7 md:px-8 md:py-9">
      ...
      </Link></> : <section className="rounded-2xl border border-[#cbded3] bg-[#f4fbf7] p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-[#1e6b51]">Live source verification</p><h3 className="mt-1 text-base font-semibold">Uploaded or connected-source evidence has been tested against approved controls.</h3><p className="mt-1 text-xs leading-5 text-[#5e7168]">Run control-quality mutations against this source, or open a root cause for bounded investigation.</p><div className="mt-4 flex flex-wrap gap-3"><Link href={`/runs/${activeRun}/mutation-test`} className="inline-flex items-center gap-2 rounded-lg bg-[#112a2b] px-3 py-2 text-xs font-semibold text-white">Run mutation tests <ArrowRight size={13} /></Link>{roots.data?.[0] ? <Link href={`/root-causes/${roots.data[0].id}`} className="inline-flex items-center gap-2 rounded-lg border border-[#cbded3] bg-white px-3 py-2 text-xs font-semibold">Investigate root cause <ArrowRight size={13} /></Link> : null}<Link href="/data" className="inline-flex items-center gap-2 rounded-lg border border-[#cbded3] bg-white px-3 py-2 text-xs font-semibold">Open data sources</Link></div></section>}
