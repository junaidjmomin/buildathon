...
export default function ExceptionsPage() {
  ...
  const runs = useQuery({ queryKey: ["runs"], queryFn: api.runs, enabled: !DEMO_MODE });
  const activeRunId = DEMO_MODE ? RUN_ID : runs.data?.[0]?.id;
  ...
  const unresolved = useQuery({ queryKey: ["unresolved", activeRunId], queryFn: () => api.unresolvedMatches(activeRunId ?? ""), enabled: DEMO_MODE && Boolean(activeRunId) });
  ...
  if ((!DEMO_MODE && runs.isPending) || cases.isPending || (DEMO_MODE && unresolved.isPending)) return <AppShell><div className="grid min-h-[calc(100vh-64px)] place-items-center"><LoaderCircle className="animate-spin text-[#1e6b51]" /></div></AppShell>;
