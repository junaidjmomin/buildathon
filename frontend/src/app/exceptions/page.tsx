...
export default function ExceptionsPage() {
  ...
  if (!active) return <AppShell><main className="mx-auto max-w-4xl p-8"><div className="panel rounded-2xl p-8"><h1 className="text-xl font-semibold">No exception cases for this run</h1><p className="mt-2 text-sm text-[#66716b]">The run may have no violations, or case generation has not produced an evidence-backed case yet. Review its outcomes on Overview.</p></div><UnresolvedQueue activeRunId={activeRunId} seeded={activeRun?.source === "SEEDED"} items={unresolved.data ?? []} /></main></AppShell>;
  ...
  return (
    <AppShell>
      <main className="mx-auto max-w-[1320px] px-5 py-8 md:px-8 md:py-10">
        ...
        <UnresolvedQueue activeRunId={activeRunId} seeded={activeRun?.source === "SEEDED"} items={unresolved.data ?? []} />
      </main>
    </AppShell>
  );
}
