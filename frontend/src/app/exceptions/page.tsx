...
export default function ExceptionsPage() {
  ...
  const activeRun = runs.data?.find((run) => run.id === selectedRunId) ?? runs.data?.find((run) => run.source !== "SEEDED" && run.status === "COMPLETE") ?? runs.data?.find((run) => run.status === "COMPLETE");
  ...
  const unresolved = useQuery({ queryKey: ["unresolved", activeRunId], queryFn: () => api.unresolvedMatches(activeRunId ?? ""), enabled: activeRun?.source === "SEEDED" && Boolean(activeRunId) });
  ...
  if (runs.isPending || cases.isPending || (activeRun?.source === "SEEDED" && unresolved.isPending)) return <AppShell><div className="grid min-h-[calc(100vh-64px)] place-items-center"><LoaderCircle className="animate-spin text-[#1e6b51]" /></div></AppShell>;
  ...
  if (!active) return <AppShell><main className="mx-auto max-w-3xl p-8"><div className="panel rounded-2xl p-8"><h1 className="text-xl font-semibold">No exception cases for this run</h1><p className="mt-2 text-sm text-[#66716b]">The run may have no violations, or case generation has not produced an evidence-backed case yet. Review its outcomes on Overview.</p></div></main></AppShell>;
