  ...
  return (
    <AppShell>
      <main className="mx-auto max-w-[1160px] px-5 py-8 md:px-8 md:py-10">
        <div className="mb-8">
          <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#1e6b51]">
            Ingestion
          </p>
          ...
          <p className="mt-2 text-sm text-[#66716b]">
            Every source normalizes into the same financial event graph and deterministic control
            pipeline.
          </p>
        </div>
        ...
        <section className="mb-6 grid gap-4 md:grid-cols-3">
          ...
          >
            <button
              ...
              className="mt-5 flex items-center gap-2 rounded-lg bg-[#112a2b] px-3 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-45"
            >
              {syncing ? (
        ...
        <section className="panel overflow-hidden rounded-2xl">
          <div className="flex flex-col justify-between gap-3 border-b border-[#e2e5df] px-5 py-4 sm:flex-row sm:items-center">
            <div>
              <h2 className="flex items-center gap-2 text-sm font-semibold">
                <WalletCards size={16} className="text-[#1e6b51]" /> Razorpay read-only
                connector
              </h2>
          ...
          <div className="p-5">
            {!status.data?.configured && (
              <div className="mb-5 flex items-start gap-3 rounded-xl border border-[#e1e5df] bg-[#f7f8f5] p-4">
                ...
                <div>
                  <p className="text-xs font-semibold">Configure credentials on the backend</p>
                  <p className="mt-1 text-[11px] leading-5 text-[#68736d]">
                    Set RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET in the backend environment. Secrets
                    are never requested by or returned to this browser.
                  </p>
                </div>
              </div>
            )}
            ...
            {(sync.error || submit.error || job.data?.error) && (
              <p className="mt-4 text-xs text-[#a43d32]" role="alert">
                {job.data?.error?.message ?? sync.error?.message ?? submit.error?.message}
              </p>
            )}
            <div className="mt-5 flex items-center gap-2 border-t border-[#e5e8e2] pt-4 text-[11px] text-[#66716b]">
              <ShieldCheck size={14} className="text-[#1e6b51]" /> Connector permissions are
        ...
        {mcp.data && (
          <section className="panel mt-6 overflow-hidden rounded-2xl">
            <div className="flex flex-col justify-between gap-3 border-b border-[#e2e5df] px-5 py-4 sm:flex-row sm:items-center">
              <div>
                <h2 className="flex items-center gap-2 text-sm font-semibold">
                  <BrainCircuit size={16} className="text-[#1e6b51]" /> Optional MCP investigation
                  evidence
                </h2>
                <p className="mt-1 text-xs text-[#78827d]">
              ...
              <span className="w-fit rounded-full bg-[#eceeed] px-2.5 py-1 text-[10px] font-bold text-[#66716b]">
                {mcp.data.enabled ? "ENABLED · READ ONLY" : "DISABLED BY DEFAULT"}
              </span>
            </div>
            <div className="p-5">
              <div className="flex flex-wrap gap-2">
                {mcp.data.allowed_tools.map((tool) => (
                  <span
                    key={tool}
                    className="rounded-md bg-[#eef2ed] px-2 py-1 font-mono text-[9px] text-[#52615a]"
                  >
                    {tool}
                  </span>
                ))}
              </div>
              <p className="mt-4 flex items-start gap-2 border-t border-[#e5e8e2] pt-4 text-[11px] leading-5 text-[#66716b]">
                <ShieldCheck size={14} className="mt-0.5 shrink-0 text-[#1e6b51]" />
  ...
  return (
    <div className="panel rounded-2xl p-5">
      <div className="flex items-start justify-between gap-3">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e7f2ec] text-[#1e6b51]">
          <Icon size={19} />
        </span>
        <span className="rounded-full bg-[#eef1ec] px-2 py-1 text-[9px] font-bold text-[#66716b]">
          {badge}
        </span>
      </div>
      <h2 className="mt-5 text-sm font-semibold">{title}</h2>
      <p className="mt-2 min-h-12 text-xs leading-5 text-[#6b7670]">{description}</p>
      {children}
    </div>
  );
}
...
function SyncMetric({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="min-w-0 rounded-xl border border-[#e2e6e0] bg-[#fafbf8] p-3">
      <p className="number-tabular truncate text-lg font-semibold" title={String(value)}>
        {value}
      </p>
      <p className="mt-1 text-[10px] text-[#758079]">{label}</p>
    </div>
  );
}
